#!/usr/bin/env bash
# Import the fleet dashboard and the Grafana-managed alert rules into Azure
# Managed Grafana (monitoring option A).
...
az grafana dashboard import -g "$RG" -n "$NAME" --overwrite true \
  --definition "@${ROOT}/monitoring/standalone/hub/dashboards/tpg-fleet.json" >/dev/null
...
echo "Done: ${ENDPOINT}/d/tpg-fleet"
